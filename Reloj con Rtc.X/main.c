...
void antiRebote(unsigned int pin);
...
void antiRebote(unsigned int pin) {
    ...
    switch (pin) {
        ...
        case 1:
            while (!BOTON_INCREMENTAR); //Y TEMPERATURA
            while (BOTON_INCREMENTAR);
            __delay_ms(RETARDO_ANTIREBOTE);
            break;

        case 2:
            while (!BOTON_DECREMENTAR); // Y FORMATO
            while (BOTON_DECREMENTAR);
            __delay_ms(RETARDO_ANTIREBOTE);
            break;
...
void dameTemperatura(void) {
    ...
    antiRebote(1); //ANTIREBOTE

    contadorBotonTemp++;

    if (contadorBotonTemp > 2) {
        contadorBotonTemp = 0;
    } else if (contadorBotonTemp == 1) {
...
void dameHoraActual(void) //RTC DS3231
{
    ...
    if ((((contDecHora >= 0) && (contDecHora <= 2)) && ((contHora >= 0) && (contHora <= 9)))
            && (((contDecMin >= 0) && (contDecMin <= 5)) && ((contMin >= 0) && (contMin <= 9)))) //Hora valida!
    {
    } else
        setRtcDefault();
...
void main(void) {
    ...
    while (1) {
        ...
        if (!contadorBotonSet) {
            ...
            if (contadorBotonTemp == 1 && (numeroActual != contMin))
                contadorBotonTemp = 0;
